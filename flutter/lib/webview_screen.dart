...
class _WebViewScreenState extends State<WebViewScreen> {
  ...
  Future<void> _loadIpConfig() async {
    ...
    final savedPort = prefs.getString('backend_port') ?? '9999';
    if (savedIp.isNotEmpty) {
      setState(() {
        _backendApiBase = 'http://$savedIp:$savedPort';
      });
    } else {
      // Default: use local monolithic port
      setState(() {
        _backendApiBase = 'http://10.0.2.2:9999';
      });
    }
  }
  ...
  void _initWebView() {
    _controller = WebViewController()
      ...
      ..loadRequest(Uri.parse(_backendApiBase));
  }
